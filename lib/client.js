...
/* conversation.input.right 的条目固定渲染在原生「模型 / 推理等级」座位左侧；
   用 flex order 把别名选择器移到工具行右端（发送按钮之前），保持与原生控件相邻。 */
*:has(> .dma-selector)>.dma-selector{order:1}
*:has(> .dma-selector)>*:last-child{order:2}
